...
class CountdownTimer {
  ...
  countTimeComponents(curDate) {
    const days = this.pad(Math.floor(
      (this.targetDate - curDate) / (1000 * 60 * 60 * 24)
    ));
    const hours =this.pad( Math.floor(
      ((this.targetDate - curDate) % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)
    ));
    const mins =this.pad( Math.floor(
      ((this.targetDate - curDate) % (1000 * 60 * 60)) / (1000 * 60)
    ));
    const secs =this.pad( Math.floor(((this.targetDate - curDate) % (1000 * 60)) / 1000));
...
const timer = new CountdownTimer({
  selector: "#timer-1",
  targetDate: new Date("Oct 10, 2021 20:32:30"),
});
